...
@pytest.fixture
def valid_genix_address(network='mainnet'):
    return 'qWrugUr2zYrKX3jkyJEWEbPHgysd3PspAH' if (network == 'testnet') else 'PDDaohBNLXomGiLuagnchbE2K3hbZTMbab'


@pytest.fixture
def invalid_genix_address(network='mainnet'):
    return 'qWrugUr2zYrKX3jkyJEWEbPHgysd3PspAI' if (network == 'testnet') else 'PDDaohBNLXomGiLuagnchbE2K3hbZTMbaC'
...
def mn_list():
    ...
    masternodelist_full = {
        u'701854b26809343704ab31d1c45abc08f9f83c5c2bd503a9d5716ef3c0cda857-1': u'  ENABLED 70201 yjaFS6dudxUTxYPTDB9BYd1Nv4vMJXm3vK 1474157572    82842 1474152618  436491 52.90.74.124:19999',
        ...
        u'656695ed867e193490261bea74783f0a39329ff634a10a9fb6f131807eeca744-1': u'  ENABLED 70201 yepN97UoBLoP2hzWnwWGRVTcWtw1niKwcB 1474157704   824622 1474152571  436490 178.62.203.249:19999',
    }
...
def test_valid_genix_address():
    from genixlib import is_valid_genix_address

    main = valid_genix_address()
    test = valid_genix_address('testnet')

    assert is_valid_genix_address(main) is True
    assert is_valid_genix_address(main, 'mainnet') is True
    assert is_valid_genix_address(main, 'testnet') is False

    assert is_valid_genix_address(test) is False
    assert is_valid_genix_address(test, 'mainnet') is False
    assert is_valid_genix_address(test, 'testnet') is True


def test_invalid_genix_address():
    from genixlib import is_valid_genix_address

    main = invalid_genix_address()
    test = invalid_genix_address('testnet')

    assert is_valid_genix_address(main) is False
    assert is_valid_genix_address(main, 'mainnet') is False
    assert is_valid_genix_address(main, 'testnet') is False

    assert is_valid_genix_address(test) is False
    assert is_valid_genix_address(test, 'mainnet') is False
    assert is_valid_genix_address(test, 'testnet') is False
...
def test_hash_function():
    ...
    hex_hash = "%x" % genixlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(genixlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(genixlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(genixlib.blocks_to_seconds(16616)) == 2612035
