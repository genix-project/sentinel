...
def is_valid_genix_address(address, network='mainnet'):
    ...
    genix_version = 120 if network == 'testnet' else 55
    ...
    try:
        ...
        # rescue from exception, not a valid Genix address
        return False

    if (address_version != genix_version):
        return False
...
# shims 'til we can fix the genixd side
def SHIM_serialise_for_genixd(sentinel_hex):
    from models import GENIXD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = GENIXD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in genixd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # genixd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    genixd_hex = serialise(obj)
    return genixd_hex


# shims 'til we can fix the genixd side
def SHIM_deserialise_from_genixd(genixd_hex):
    from models import GENIXD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(genixd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('genix.conf')
