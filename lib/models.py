...
class GovernanceObject(BaseModel):
    ...
    # sync genixd gobject list with our local relational DB backend
    @classmethod
    def sync(self, genixd):
        golist = genixd.rpc_command('gobject', 'list')
    ...
    def import_gobject_from_genixd(self, genixd, rec):
        ...
        # exclude any invalid model data from genixd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from genixd, with every run
        ...
        # get/create, then sync payment amounts, etc. from genixd - Genixd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(genixd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from genixd! %s" % e)
            govobj.vote_delete(genixd)
            return (govobj, None)
    ...
    def vote_delete(self, genixd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(genixd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, genixd, signal, outcome):
        import genixlib
        ...
        output = genixd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = genixlib.did_we_vote(output)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 genix addr, non-multisig
            if not genixlib.is_valid_genix_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Genix address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (genixlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = GENIXD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not genixlib.is_valid_genix_address(addr, config.network):
    ...
    def hash(self):
        import genixlib
        return genixlib.hashit(self.serialise())
