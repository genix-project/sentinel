...
def has_genix_conf():
    ...
    valid_genix_conf = False

    # ensure genix_conf exists & readable
    #
    # if not, print a message stating that Genix Core must be installed and
    # configured, including JSONRPC access in genix.conf
    try:
        f = io.open(config.genix_conf)
        valid_genix_conf = True
    except IOError as e:
        print(e)

    return valid_genix_conf
...
def main():
    ...
    if not has_genix_conf():
        print("GenixCore must be installed and configured, including JSONRPC access in genix.conf")
        sys.exit(1)
